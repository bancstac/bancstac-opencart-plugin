...
class ModelExtensionPaymentPaycorp extends Model {
	...
	/* public function getCards($customer_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "payment_paycorp_card WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function addOrder($order_info, $order_code) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "payment_paycorp_order` SET `order_id` = '" . (int)$order_info['order_id'] . "', `order_code` = '" . $this->db->escape($order_code) . "', `date_added` = now(), `date_modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");

		return $this->db->getLastId();
	}

	public function getOrder($order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "payment_paycorp_order` WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");

		if ($qry->num_rows) {
			$order = $qry->row;
			$order['transactions'] = $this->getTransactions($order['payment_paycorp_order_id']);
	...
	public function addTransaction($payment_paycorp_order_id, $type, $order_info) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "payment_paycorp_order_transaction` SET `payment_paycorp_order_id` = '" . (int)$payment_paycorp_order_id . "', `date_added` = now(), `type` = '" . $this->db->escape($type) . "', `amount` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
	}

	public function getTransactions($payment_paycorp_order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "payment_paycorp_order_transaction` WHERE `payment_paycorp_order_id` = '" . (int)$payment_paycorp_order_id . "'");
	...
	public function recurringPayment($item, $order_id_rand, $token) {
		...
		$this->model_extension_payment_paycorp->logger($order);

		$response_data = $this->model_extension_payment_paycorp->sendCurl('orders', $order);

		$this->model_extension_payment_paycorp->logger($response_data);
	...
	public function cronPayment() {
		...
		$log = new Log('payment_paycorp_recurring_orders.log');
	...
	public function getpaycorpOrder($payment_paycorp_order_id) {
		$qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "payment_paycorp_order WHERE order_code = " . (int)$payment_paycorp_order_id);
		return $qry->row;
	}

	public function updateCronJobRunTime() {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'paycorp' AND `key` = 'payment_paycorp_last_cron_job_run'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'paycorp', 'payment_paycorp_last_cron_job_run', NOW(), 0)");
	}

	public function sendCurl($url, $order = null) {
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, 'https://api.paycorp.com/v1/' . $url);
		$content_length = 0;
		if ($order) {
		...
		curl_setopt(
			...
			"Authorization: " . $this->config->get('payment_paycorp_service_key'),
	...
	public function logger($data) {
		if ($this->config->get('payment_paycorp_debug')) {
			$log = new Log('payment_paycorp_debug.log');
